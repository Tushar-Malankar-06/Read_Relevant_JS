...
const PROMPT_PATH = path.join(__dirname, '../prompts/batch.txt');
...
async function processLatestFile() {
  ...
  const latestFile = getLatestFile('thebatch');
  ...
  try {
    ...
    const dateMatch = latestFile.match(/thebatch_(\d{8})/);
    const dateStr = dateMatch ? dateMatch[1] : 'latest';
    const outPath = path.join(OUTPUT_DIR, `thebatch_${dateStr}.json`);
