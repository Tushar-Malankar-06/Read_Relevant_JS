...
const PROMPT_PATH = path.join(__dirname, '../prompts/unwind.txt');
...
async function processLatestFile() {
  ...
  const latestFile = getLatestFile('unwindai');
  ...
  try {
    ...
    const dateMatch = latestFile.match(/unwindai_(\d{8})/);
    const dateStr = dateMatch ? dateMatch[1] : 'latest';
    const outPath = path.join(OUTPUT_DIR, `unwindai_${dateStr}.json`);
