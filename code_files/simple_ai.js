...
const PROMPT_PATH = path.join(__dirname, '../prompts/simple_ai.txt');
...
async function processLatestFile() {
  ...
  const latestFile = getLatestFile('simple_ai');
  ...
  try {
    ...
    const dateMatch = latestFile.match(/simple_ai_(\d{8})/);
    const dateStr = dateMatch ? dateMatch[1] : 'latest';
    const outPath = path.join(OUTPUT_DIR, `simple_ai_${dateStr}.json`);
