...
const PROMPT_PATH = path.join(__dirname, '../prompts/ai_valley.txt');
...
async function processLatestFile() {
  ...
  const latestFile = getLatestFile('aivalley');
  ...
  try {
    ...
    const dateMatch = latestFile.match(/aivalley_(\d{8})/);
    const dateStr = dateMatch ? dateMatch[1] : 'latest';
    const outPath = path.join(OUTPUT_DIR, `aivalley_${dateStr}.json`);
