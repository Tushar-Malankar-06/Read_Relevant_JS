...
const PROMPT_PATH = path.join(__dirname, '../prompts/rundown_daily.txt');
...
async function processLatestFile() {
  ...
  const latestFile = getLatestFile('therundown_daily');
  ...
  try {
    ...
    const dateMatch = latestFile.match(/therundown_daily_(\d{8})/);
    const dateStr = dateMatch ? dateMatch[1] : 'latest';
    const outPath = path.join(OUTPUT_DIR, `therundown_daily_${dateStr}.json`);
