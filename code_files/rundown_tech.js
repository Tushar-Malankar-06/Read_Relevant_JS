...
const PROMPT_PATH = path.join(__dirname, '../prompts/rundown_tech.txt');
...
async function processLatestFile() {
  ...
  const latestFile = getLatestFile('therundown_technews');
  ...
  try {
    ...
    const dateMatch = latestFile.match(/therundown_technews_(\d{8})/);
    const dateStr = dateMatch ? dateMatch[1] : 'latest';
    const outPath = path.join(OUTPUT_DIR, `therundown_technews_${dateStr}.json`);
