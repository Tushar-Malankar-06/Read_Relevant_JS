...
const PROMPT_PATH = path.join(__dirname, '../prompts/mindstream.txt');
...
async function processLatestFile() {
  ...
  const latestFile = getLatestFile('mindstream');
  ...
  try {
    ...
    const dateMatch = latestFile.match(/mindstream_(\d{8})/);
    const dateStr = dateMatch ? dateMatch[1] : 'latest';
    const outPath = path.join(OUTPUT_DIR, `mindstream_${dateStr}.json`);
